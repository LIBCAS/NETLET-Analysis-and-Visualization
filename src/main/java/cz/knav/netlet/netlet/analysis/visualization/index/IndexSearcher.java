...
public class IndexSearcher {
  ...
  public static JSONObject getLetterFromHIKO(String id, String tenant) {
    ...
    try (HttpClient httpclient = HttpClient
      ...
      //r = response.body();
      ret = new JSONObject(response.body()).getJSONObject("data");
  ...
  public static JSONObject getLetter(String id) {
    ...
    try (SolrClient solr = new HttpJdkSolrClient.Builder(Options.getInstance().getString("solr")).build()) {
      ...
      final JsonQueryRequest jrequest = new JsonQueryRequest()
              .setQuery("*:*")
              .withFilter("id:" + id)
              .returnFields("*,places:[json],identities:[json],professions:[json],abstract:[json],keywords:[json]")
              .setLimit(1);
...
//              .withFacet("keywords", categoriesFacet)
              .withFacet("chart_mentioned", new TermsFacetMap("identity_mentioned")
                      ...
                      .setSort("index")
                      .setMinCount(1))
              .withFacet("chart_recipients", new TermsFacetMap("identity_recipient")
                      .setLimit(1000)
                      .setSort("index")
                      .setMinCount(1))
              .withFacet("chart_authors", new TermsFacetMap("identity_author")
                      .setLimit(1000)
                      .setSort("index")
                      .setMinCount(1))
              .setLimit(rows);
  ...
  public static JSONObject professions(HttpServletRequest request) {
    ...
    try (SolrClient solr = new HttpJdkSolrClient.Builder(Options.getInstance().getString("solr")).build()) {
      ...
      JsonQueryRequest jrequest = new JsonQueryRequest()
              ...
              //.withFilter("status:publish")
              .setLimit(rows)
              .withFilter("professions_recipient_" + lang + ":* OR professions_author_" + lang + ":*")
              .returnFields("id,date_year,identity_name,identity_recipient,identity_author,origin,destination,identities:[json],professions:[json]")
              .withFacet("date_year", rangeFacet)
              .withFacet("tenants", new TermsFacetMap("tenant")
                      .setLimit(1000)
...
//    Date until = dtformatter.parse(years[1]);
  ...
  public static JsonQueryRequest addFilters(HttpServletRequest request, JsonQueryRequest jrequest, String lang, boolean excludeDateRange) {
    ...
    if ((year_from != null && !year_from.isBlank()) || (year_to != null && !year_to.isBlank())) {
      if (year_from == null || year_from.isBlank()) {
        year_from = "*";
      }
      if (year_to == null || year_to.isBlank()) {
        year_to = "*";
      }
